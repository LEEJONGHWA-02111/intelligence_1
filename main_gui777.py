...
class Application:
    ...
    def stop_button(self):
        """STOP button callback: Force stop the conveyor in any case."""
        # Set emergency_stop flag and clear any waiting event
        self.emergency_stop = True
        self.waiting_for_go.clear()
        self.log("Emergency STOP pressed: Stopping conveyor")
        try:
            # Send stop command twice to ensure the command is received
            self.ser.write(b"0")
            time.sleep(0.1)
            self.ser.write(b"0")
        except Exception as e:
