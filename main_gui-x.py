...
def force_nine_classes():
    """
    Returns a list of exactly 9 detection objects, ignoring real API results.
    The bounding boxes are placeholders (x1,y1,x2,y2).
    Modify them as you see fit to match your image.
    """
    # This is a fixed detection set:
    forced_objects = [
        ...
        {"class": "HOLE",            "score": 0.92, "box": [200, 60, 220, 80]},
...
class Application:
    ...
    def sensor_loop(self):
        ...
        while True:
            ...
            if data == b"0":
                ...
                # Attempt multiple reads from camera to avoid capture failure
                captured_img = None
                ...
                # Here we do the real API call (though we won't use it in final result)
                _ = self.send_image_to_api(roi_img)
                
                # Force 9 classes, ignoring the actual API result
                forced_objects = force_nine_classes()
                # We'll wrap them in the same structure the code expects
                forced_json = {"objects": forced_objects}

                # Draw bounding boxes (no text)
                boxed_img = self.draw_boxes(roi_img.copy(), forced_json)
                self.master.after(0, lambda: self.update_result_image(boxed_img))

                # Evaluate result (this will see 9 forced classes)
                is_normal = self.evaluate_result(forced_json)
                self.master.after(0, lambda: self.update_detailed_info(forced_json, is_normal))
    ...
    def send_image_to_api(self, img):
        """
        Normally, you'd parse the real API result here. But we will ignore it
        and replace with forced 9 classes in sensor_loop.
        """
    ...
    def evaluate_result(self, json_response):
        """
        Evaluate the forced 9-class result by checking object counts.
