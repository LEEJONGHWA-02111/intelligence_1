...
def enhance_image(img):
    ...
    이미지 전처리: 
      ...
      2) Gamma Correction(1.1)으로 밝기/어두움 완화
      3) 약화된 샤프닝 필터
    """
    # 1) LAB 변환 후 CLAHE
...
class Application:
    ...
    def evaluate_result(self, json_response):
        ...
        Evaluate the API result by:
          1) filtering out objects with low confidence (< 0.5),
          2) if any class ends with "X" (e.g. "HOLE X"), immediately fail,
          3) otherwise, check if the remaining normal classes match EXACTLY the EXPECTED_COUNTS.
        """
        min_conf = 0.5  # 점수 임계값(예: 0.5)

        # 1) confidence가 낮은 객체는 제외
        objects = [obj for obj in json_response.get("objects", []) 
                   if obj.get("score", 0.0) >= min_conf]

        # 2) "X"로 끝나는 클래스가 하나라도 있으면 FAIL
        for obj in objects:
            class_name = obj.get("class", "")
            if class_name.strip().endswith("X"):
                return False  # 즉시 FAIL

        # 3) 남은 클래스들에 대해 EXPECTED_COUNTS와 정확히 일치해야 SUCCESS
        counts = {}
