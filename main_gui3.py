...
# 기존 정상 판정을 위한 기대 객체 갯수 (평가 기준은 기존과 동일)
EXPECTED_COUNTS = {
    "RASPEBBRY PICO": 1,
    "HOLE": 4,
    "CHIPSET": 1,
...
# 새로 추가된 클래스들은 이미지에 빨간색으로 표시 (BGR: (0, 0, 255))
...
def define_roi():
    ...
    ROI 영역 정의 함수
    Returns:
        dict: {x, y, width, height} 형태의 관심 영역
    """
    # 컨베이어 벨트 영역에 해당하는 ROI (예: x=200, y=100, width=400, height=280)
    return {"x": 200, "y": 100, "width": 400, "height": 280}

def crop_to_roi(img, roi):
    """
    이미지에서 ROI 영역만 추출하는 함수.
    Args:
        img (numpy.array): 원본 이미지
        roi (dict): {x, y, width, height} 형태의 관심 영역
    Returns:
        numpy.array: ROI 영역만 잘라낸 이미지
    """
    x = roi["x"]
    y = roi["y"]
    w = roi["width"]
...
class Application:
    ...
    def update_live_feed(self):
        ...
        if ret:
            # 전체 프레임 대신 ROI 영역만 추출하여 표시 (실시간 모니터링용)
    ...
    def sensor_loop(self):
        """시리얼 포트에서 센서 신호 읽기 및 ROI 영역 이미지를 캡처, 처리"""
        while True:
            ...
            if data == b"0":
                ...
                self.master.after(0, lambda: self.log("ROI 이미지 캡처 완료. API 전송 중..."))
                # API 요청 (blocking 호출)
                ...
                if self.evaluate_result(result_json):
                    ...
                    self.waiting_for_go.wait()  # GO 버튼 클릭 시 이벤트 발생
    ...
    def draw_boxes(self, img, json_response):
        ...
        for obj in objects:
            ...
            # 새로 추가된 클래스는 "X"가 붙은 이름으로 판별 → 빨간색으로 표시
    ...
    def evaluate_result(self, json_response):
        ...
        평가 시 새로 추가된 클래스("X"가 붙은 클래스)는 평가 대상에서 제외합니다.
        """
        counts = {}
