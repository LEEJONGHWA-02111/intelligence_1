...
class Application:
    ...
    def sensor_loop(self):
        ...
        while True:
            ...
            if data == b"0":
                ...
                # 박스 그리기: API 결과 기반 (새 클래스도 포함하여 모두 박스 처리)
    ...
    def draw_boxes(self, img, json_response):
        ...
        for obj in objects:
            ...
            if class_name.strip().endswith("X"):
                ...
                # 기존 클래스의 경우, 대소문자 혼용 가능하므로 upper()로 매핑
    ...
    def evaluate_result(self, json_response):
        ...
        평가 시 새로 추가된 클래스("X"가 붙은 클래스)는 무시하고 기존 EXPECTED_COUNTS에 해당하는
        객체만 정확한 갯수여야 정상 판정.
        """
        counts = {}
        # 평가를 위해 기대 클래스는 대문자 변환 후 집합 생성
        expected_set = set([k.upper() for k in EXPECTED_COUNTS.keys()])
        for obj in json_response.get("objects", []):
            cls = obj.get("class", "")
            # 새로 추가된 클래스는 평가 대상에서 제외
